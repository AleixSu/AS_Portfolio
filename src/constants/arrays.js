export const logoArrays = [
  './images/html-5.png',
  './images/css-3.png',
  './images/javaScriptLogo.png',
  './images/nodeLogo.png',
  './images/gitHubLogo.png',
  './images/React-icon.svg.png',
  './images/cloudinaryLogo.png',
  './images/expressLogo.png',
  './images/JWTLogo.png',
  './images/mongodblogo.png',
  './images/TQLogo.png',
  './images/mongoose.png'
]

export const projectArray = [
  {
    projectName: 'API Pinterest Model',
    projectVID: './vids/pinterestModel.mp4',
    projectGH: 'https://github.com/AleixSu/API-Pinterest-Exercise.git',
    projectURL: ''
  },
  {
    projectName: 'GameHub',
    projectVID: './vids/gameHub.mp4',
    projectGH: 'https://github.com/AleixSu/Proyecto-05.GameHub.git',
    projectURL: ''
  },
  {
    projectName: 'Portfolio',
    projectVID: './vids/portfolio.mp4',
    projectGH: 'Project number 3',
    projectURL: 'Project number 2'
  },
  {
    projectName: 'Eventia Web',
    projectVID: './vids/eventiaWeb.mp4',
    projectGH: 'Project number 3',
    projectURL: 'Project number 3'
  }
]
